      ...
      <div className="bg-gradient-to-r from-blue-600 to-blue-500 px-5 py-3 flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2 text-white">
            ...
            <span className="text-[10px] font-bold bg-white/20 px-2 py-0.5 rounded-full tracking-wide">LIVE AI · GPT-4o</span>
          </div>
          <p className="text-[11px] text-blue-200 mt-0.5">Auto-generated from patient intake · flags allergies, gaps, and risk factors</p>
        </div>
        <Button
          size="sm"
          onClick={onGenerate}
          disabled={loading}
          className="h-7 text-xs bg-white/15 hover:bg-white/25 text-white border border-white/25 shadow-none gap-1.5 flex-shrink-0"
        >
          {loading
      ...
      <div className="px-5 py-4 min-h-[64px]">
        {loading && (
          <div className="flex items-center gap-2.5 text-slate-500 text-sm">
            ...
            Generating AI clinical summary from patient intake data…
          </div>
        )}
        {!loading && summary && (
          <>
            <p className="text-sm text-slate-700 leading-relaxed">{summary}</p>
            <p className="text-[11px] text-slate-400 mt-3 border-t border-slate-50 pt-3">Generated by GPT-4o from patient-maintained intake. Always verify critical details with the patient directly.</p>
          </>
        )}
        {!loading && error && (
